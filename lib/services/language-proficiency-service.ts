...
export class LanguageProficiencyService {
  ...
  private static getStaticQuestions(languageCode: string, filters?: any): TestQuestion[] {
    ...
    if (languageCode === 'es') {
      const { SPANISH_PROFICIENCY_QUESTIONS } = require('@/lib/data/spanish-proficiency-questions');
      let questions = SPANISH_PROFICIENCY_QUESTIONS;
    ...
    if (languageCode === 'it') {
      const { ITALIAN_PROFICIENCY_QUESTIONS } = require('@/lib/data/italian-proficiency-questions');
      let questions = ITALIAN_PROFICIENCY_QUESTIONS;
