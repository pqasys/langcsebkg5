...
export async function PUT(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.courseMonthlyPrice.deleteMany({
        where: {
          courseId: params.id,
          year: year
        }
      ...
      // Create new prices
      const monthlyPrices = await tx.courseMonthlyPrice.createMany({
    ...
    const updatedPrices = await prisma.courseMonthlyPrice.findMany({
      where: {
        courseId: params.id,
        year: year
      },
