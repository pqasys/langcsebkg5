...
import { ArrowLeft, Save, Plus, Trash2, Upload, Image, Video, Music, Move, Target, FileText, CheckSquare, AlignLeft, Settings, Calculator, Eye } from 'lucide-react';
...
interface AdvancedQuestionData {
  type: string;
  question: string;
  points: number;
  options: string[];
...
const calculateIRTParameters = (difficulty: string, type: string, options?: string[]): IRTParameters => {
  ...
  switch (type) {
    case 'MULTIPLE_CHOICE':
      // Adjust guessing based on number of options
...
export default function AdminAddQuestionPage({ params }: { params: { id: string; moduleId: string; quizId: string } }) {
  ...
  const [questionData, setQuestionData] = useState<AdvancedQuestionData>({
  ...
  useEffect(() => {
    if (!questionData.use_manual_irt) {
      const irtParams = calculateIRTParameters(questionData.difficulty, questionData.type, questionData.options);
  ...
  }, [questionData.difficulty, questionData.type, questionData.options, questionData.use_manual_irt]);
  ...
  useEffect(() => {
    ...
    if (searchParams.get('fromTemplate') === '1') {
      const templateStr = localStorage.getItem('questionTemplate');
      if (templateStr) {
        try {
          ...
          setQuestionData((prev) => ({
            ...
            question: template.template_config?.question || '',
            points: template.template_config?.points || 1,
            options: template.template_config?.options || ['', '', '', ''],
            correct_answer: template.template_config?.correct_answer || '',
            explanation: template.template_config?.explanation || '',
            difficulty: template.difficulty || 'MEDIUM',
            category: template.category || '',
            hints: template.template_config?.hints || '',
            question_config: template.template_config || null,
            media_url: template.template_config?.media_url || '',
            media_type: template.template_config?.media_type || '',
            question_options: template.template_config?.question_options || []
          }));
        } catch (e) {
          // ignore
        }
  ...
  const handleSave = async () => {
    ...
    if (questionData.use_manual_irt) {
      const irtParams = {
        difficulty: questionData.irt_difficulty || 0,
        discrimination: questionData.irt_discrimination || 1,
        guessing: questionData.irt_guessing || 0.25
      };
    ...
    try {
      setSaving(true);
      const response = await fetch(`/api/admin/courses/${params.id}/modules/${params.moduleId}/quizzes/${params.quizId}/questions`, {
        method: 'POST',
        headers: {
      ...
      router.push(`/admin/courses/${params.id}/modules/${params.moduleId}/quizzes/${params.quizId}/questions`);
    } catch (error) {
            console.error('Error occurred:', error);
      toast.error(`Failed to saving question:. Please try again or contact support if the problem persists.`);
              ...
              {questionData.options.map((option, index) => (
                <div key={index} className="flex items-center space-x-2">
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => updateQuestionField('correct_answer', option)}
                    className={questionData.correct_answer === option ? 'bg-green-100 border-green-500' : ''}
                  >
                    Correct
                  </Button>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Switch
            checked={questionData.use_manual_irt}
            onCheckedChange={(checked) => updateQuestionField('use_manual_irt', checked)}
          />
        </div>

        {!questionData.use_manual_irt && (
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div>
                <span className="font-medium">Difficulty:</span> {questionData.irt_difficulty?.toFixed(2) || '0.00'}
              </div>
              <div>
                <span className="font-medium">Discrimination:</span> {questionData.irt_discrimination?.toFixed(2) || '1.00'}
              </div>
              <div>
                <span className="font-medium">Guessing:</span> {questionData.irt_guessing?.toFixed(2) || '0.25'}
              </div>
            </div>
            <p className="text-xs text-blue-600 mt-2">
              Based on difficulty: {questionData.difficulty}, type: {questionData.type}
              {questionData.type === 'MULTIPLE_CHOICE' && ` (${questionData.options.filter(o => o.trim()).length} options)`}
            </p>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="-4"
                  max="4"
                  value={questionData.irt_difficulty || 0}
                  onChange={(e) => updateQuestionField('irt_difficulty', parseFloat(e.target.value) || 0)}
                  placeholder="0.0"
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0.1"
                  max="3"
                  value={questionData.irt_discrimination || 1}
                  onChange={(e) => updateQuestionField('irt_discrimination', parseFloat(e.target.value) || 1)}
                  placeholder="1.0"
                />
                <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  max="1"
                  value={questionData.irt_guessing || 0.25}
                  onChange={(e) => updateQuestionField('irt_guessing', parseFloat(e.target.value) || 0.25)}
                  placeholder="0.25"
                />
                <p className="text-xs text-muted-foreground">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Add a new question to "{quiz.title}" • {module.title} • {course.title} • {course.institution.name}
            </p>
          </div>
        </div>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <QuestionPreview 
                question={{
                  ...
                  question_config: questionData.question_config,
                  media_url: questionData.media_url,
                  media_type: questionData.media_type,
                  questionOptions: questionData.question_options
                }} 
              ...
              {questionData.media_url && (
                <div className="space-y-2">
                  ...
                  <div className="flex items-center space-x-2 p-2 bg-gray-50 rounded">
                    {questionData.media_type === 'image' && <Image className="w-4 h-4" />}
                    {questionData.media_type === 'video' && <Video className="w-4 h-4" />}
                    {questionData.media_type === 'audio' && <Music className="w-4 h-4" />}
                    <span className="text-sm truncate">{questionData.media_url}</span>
                  </div>
                </div>
              )}
            </CardContent>
