...
// Calculate the exact distribution needed for 160 questions
// A1: 10 (5 grammar + 5 vocabulary) - REAL
// A2: 10 (5 grammar + 5 vocabulary) - REAL  
// B1: 27 (14 grammar + 13 vocabulary) - PLACEHOLDER
// B2: 27 (14 grammar + 13 vocabulary) - PLACEHOLDER
// C1: 27 (14 grammar + 13 vocabulary) - PLACEHOLDER
// C2: 27 (14 grammar + 13 vocabulary) - PLACEHOLDER
// Total: 10 + 10 + 27 + 27 + 27 + 27 = 128 + 32 = 160

// Wait, that's only 128. Let me recalculate:
// We need 160 total, so we need 150 placeholder questions (160 - 10 A1 - 10 A2)
// 150 / 4 levels (B1, B2, C1, C2) = 37.5 per level
// Let's do 38 per level: 38 * 4 = 152 + 20 real = 172 (too many)
// Let's do 37 per level: 37 * 4 = 148 + 20 real = 168 (still too many)
// Let's do 36 per level: 36 * 4 = 144 + 20 real = 164 (still too many)
// Let's do 35 per level: 35 * 4 = 140 + 20 real = 160 (perfect!)

console.log('Calculating exact distribution for 160 questions:');
console.log('A1: 10 questions (5 grammar + 5 vocabulary) - REAL');
console.log('A2: 10 questions (5 grammar + 5 vocabulary) - REAL');
console.log('B1: 35 questions (18 grammar + 17 vocabulary) - PLACEHOLDER');
console.log('B2: 35 questions (18 grammar + 17 vocabulary) - PLACEHOLDER');
console.log('C1: 35 questions (18 grammar + 17 vocabulary) - PLACEHOLDER');
console.log('C2: 35 questions (18 grammar + 17 vocabulary) - PLACEHOLDER');
console.log('Total: 10 + 10 + 35 + 35 + 35 + 35 = 160 questions');

// Real A1 and A2 questions (same as before)
...
// Generate exactly 35 placeholder questions per level (B1-C2)
function generatePlaceholderQuestions() {
  const questions = [];
  
  // B1 Level (35 questions: 18 Grammar, 17 Vocabulary)
  for (let i = 1; i <= 18; i++) {
    questions.push({
      "id": `B1-G-${i}`,
      "level": "B1",
  ...
  // B2 Level (35 questions: 18 Grammar, 17 Vocabulary)
  for (let i = 1; i <= 18; i++) {
    questions.push({
      "id": `B2-G-${i}`,
      "level": "B2",
  ...
  // C1 Level (35 questions: 18 Grammar, 17 Vocabulary)
  for (let i = 1; i <= 18; i++) {
    questions.push({
      "id": `C1-G-${i}`,
      "level": "C1",
  ...
  // C2 Level (35 questions: 18 Grammar, 17 Vocabulary)
  for (let i = 1; i <= 18; i++) {
    questions.push({
      "id": `C2-G-${i}`,
      "level": "C2",
...
const placeholderQuestions = generatePlaceholderQuestions();
const b1Count = placeholderQuestions.filter(q => q.level === 'B1').length;
const b2Count = placeholderQuestions.filter(q => q.level === 'B2').length;
const c1Count = placeholderQuestions.filter(q => q.level === 'C1').length;
const c2Count = placeholderQuestions.filter(q => q.level === 'C2').length;

console.log(`\n📋 Placeholder distribution:`);
console.log(`   B1: ${b1Count} questions (18 grammar, 17 vocabulary)`);
console.log(`   B2: ${b2Count} questions (18 grammar, 17 vocabulary)`);
console.log(`   C1: ${c1Count} questions (18 grammar, 17 vocabulary)`);
console.log(`   C2: ${c2Count} questions (18 grammar, 17 vocabulary)`);
