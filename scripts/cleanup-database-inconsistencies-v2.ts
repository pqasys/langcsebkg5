...
async function cleanupDatabaseInconsistenciesV2() {
  try {
    console.log('Starting database cleanup and consolidation (Version 2)...');
    ...
    for (const [name, group] of institutionGroups) {
      if (group.length > 1) {
        ...
        for (const duplicate of duplicates) {
          ...
          // Get all courses from duplicate institution
          ...
          console.log(`    Found ${duplicateCourses.length} courses to migrate`);
          
          // Process each course individually to handle unique constraints
          for (const course of duplicateCourses) {
            // Check if a course with the same title already exists in primary institution
            const existingCourse = await prisma.course.findFirst({
              where: {
                institutionId: primary.id,
                title: course.title
              }
            });
            
            if (existingCourse) {
              console.log(`    Course "${course.title}" already exists in primary, merging enrollments...`);
              
              // Move enrollments from duplicate course to existing course
              const updatedEnrollments = await prisma.studentCourseEnrollment.updateMany({
                where: { courseId: course.id },
                data: { courseId: existingCourse.id }
              });
              
              console.log(`      Moved ${updatedEnrollments.count} enrollments`);
              ...
              console.log(`      Deleted duplicate course ${course.id}`);
              
            } else {
              // Move the course to primary institution
              await prisma.course.update({
                where: { id: course.id },
                data: { institutionId: primary.id }
              });
              
              console.log(`      Moved course "${course.title}" to primary institution`);
            }
          }
...
cleanupDatabaseInconsistenciesV2(); 