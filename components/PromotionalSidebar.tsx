  ...
  useEffect(() => {
    const fetchPromotionalContent = async () => {
      try {
        ...
        const institutionsResponse = await fetch('/api/institutions/public?featured=true&limit=2');
        const institutions: InstitutionPromotion[] = institutionsResponse.ok ? await institutionsResponse.json() : [];
        
        // Fetch high-priority courses
        const coursesResponse = await fetch('/api/courses/public?priority=high&limit=2');
        ...
        // Add course promotions
        courses.forEach(course => {
          items.push({
            id: course.id,
            type: 'course',
            title: course.title,
            description: `${course.institution.name} • ${course.duration}h • ${course.level}`,
            ctaText: 'View Course',
            ...
            priority: course.isPremiumPlacement ? 1000 : course.isFeaturedPlacement ? 800 : course.isHighCommission ? 600 : 400,
    ...
    console.error('Error occurred:', error);
        // toast.error(`Failed to load promotional content. Please try again or contact support if the problem persists.`);
      } finally {
        setLoading(false);
      }
    };
