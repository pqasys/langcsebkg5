...
interface VideoSessionFormData {
  title: string;
  description: string;
  sessionType: string;
  language: string;
...
export function VideoSessionCreate() {
  const { data: session } = useSession();
  const { userType, canAccessLiveClasses } = useSubscription();
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<VideoSessionFormData>({
    title: '',
    description: '',
    sessionType: 'GROUP',
    language: 'en',
  ...
  const handleInputChange = (field: keyof VideoSessionFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Create Video Session</h1>
          <p className="text-gray-600">
            Create a new interactive language learning session
          </p>
        </div>
      </div>
